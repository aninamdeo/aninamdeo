...
<div class="max-w-3xl">
    ...
    <form action="{{ route('admin.blogs.update', $blog) }}" method="POST" enctype="multipart/form-data" class="rounded-2xl bg-slate-800 border border-slate-700/50 p-8 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="excerpt" rows="3"
                      class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-indigo-500 resize-none">{{ old('excerpt', $blog->excerpt) }}</textarea>
        ...
        <div>
            ...
            <textarea name="content" rows="10" required
                      class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-indigo-500 resize-y">{{ old('content', $blog->content) }}</textarea>
        ...
        <div class="grid sm:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $blog->category) }}" placeholder="e.g. Laravel, AI, Tips"
                       class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-indigo-500">
            ...
            <div>
                ...
                <input type="text" name="tags" value="{{ old('tags', $blog->tags) }}" placeholder="Comma-separated tags"
                       class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-indigo-500">
        ...
        <div>
            ...
            @if($blog->image)
                <div class="mb-3">
                    <img src="{{ asset('storage/' . $blog->image) }}" alt="Current image" class="h-32 w-auto rounded-xl object-cover border border-slate-700">
        ...
        <div class="rounded-xl bg-slate-900/50 border border-slate-700 p-5 space-y-4">
            ...
            <div>
                ...
                <textarea name="meta_description" rows="2"
                          class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-indigo-500 resize-none">{{ old('meta_description', $blog->meta_description) }}</textarea>
            </div>
        </div>
        ...
        <div class="grid sm:grid-cols-2 gap-5 items-end">
            <div>
                ...
                <input type="date" name="published_at" value="{{ old('published_at', $blog->published_at?->format('Y-m-d')) }}"
                       class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-indigo-500">
            </div>
            <div class="flex items-center gap-3 pb-3">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_published" value="1" {{ old('is_published', $blog->is_published) ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600">
