...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.services.update', $service) }}" method="POST" class="rounded-2xl bg-slate-800 border border-slate-700/50 p-8 space-y-5">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <textarea name="description" rows="4" required
                      class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:border-indigo-500 resize-none">{{ old('description', $service->description) }}</textarea>
        ...
        <div class="grid sm:grid-cols-3 gap-5">
            ...
            <div class="flex items-center gap-3 pt-7">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $service->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600">
